            ...
            steps {
                script {
                        try {
                            ...
                            def response = sh(script: 'curl -L -X POST -H "Accept: application/vnd.github.v3+json" -H "Authorization: Bearer $GH_TOKEN" -H "X-GitHub-Api-Version: 2022-11-28" -d \'' + payload + '\' ' + url, returnStdout: true).trim()
