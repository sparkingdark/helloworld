            ...
            steps {
                script {
                    try {
                        ...
                        def response = sh(script: 'curl -L  -X POST  -H "Accept: application/vnd.github+json"  -H "Authorization: Bearer $GH_TOKEN" -H "X-GitHub-Api-Version: 2022-11-28" ${url} -d '{"ref":"main",}'', returnStdout: true).trim()
